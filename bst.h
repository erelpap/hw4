...
protected:
    ...
    int height(Node<Key, Value> *r) const;
		bool isChildBalanced(Node<Key, Value>* child) const;
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    // Node<Key, Value> result(keyValuePair.first, keyValuePair.second, nullptr);
		

	if(root_ == nullptr){
			root_ = new Node<Key, Value> (keyValuePair.first, keyValuePair.second, nullptr);
			return;
			// can not change root_ correctly		
	}       
...
void BinarySearchTree<Key, Value>::removeHelper(Node<Key, Value>* temp)
{
   if(temp == nullptr){
		...
		//finding if the removed node is right or left child to connect the tree correctly after removal
		...
		if(!root && (temp->getParent())->getRight() == temp){
			rightChild = true;
		}
		else if(!root && (temp->getParent())->getLeft() == temp){
			leftChild = true;
		}

		//if removed node has no children
		...
		//if removed node has only right child
		...
		//if removed node has 2 children
			...
			// first nodeSwap with predecessor
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
    ...
    while(exists){
        ...
        else if(temp->getKey() >= key && temp->getLeft() != nullptr){
            temp = temp->getLeft();
        }
        else if(temp->getKey() <= key && temp->getRight() != nullptr){
            temp = temp->getRight();
        }
        else{
            exists = false;
...
// helper for isBalanced (copy pasted from part 2)
template<typename Key, typename Value>
...
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isChildBalanced(Node<Key, Value>* child) const
{
	if(child == nullptr){
		return true;
	}

	else if(height(child->getLeft()) - height(child->getRight()) <= 1 && height(child->getLeft()) - height(child->getRight()) >= -1){
		if(isChildBalanced(child->getRight()) && isChildBalanced(child->getLeft())){
			return true;
		}
	}
